...
export function TimelineTab() {
  ...
  const { availableTimelines, timelineScales, timelinesLoading } = state
  ...
  const handleCreateTimeline = async () => {
    ...
    try {
      // A timeline is just a name — tiling is adaptive: every entry indexes
      // at its own precision ('1769' → year cells, '1769-08-15' → day cells).
        ...
        <div>
              ...
              {domainTimelines.map(name => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Observed tiers — what this timeline's data has
                        materialized. Informational: tiling is adaptive, every
                        entry indexes at its own precision. */}
                    {(timelineScales[name]?.length ?? 0) > 0 && (
                      <span
                        className="shrink-0 text-[10px] px-1 py-0.5 rounded bg-muted text-muted-foreground select-none"
                        title={`Observed granularity for "${name}" — each entry is indexed at its own precision (a bare year at year cells, a full date at day cells)`}
                      >
                        {(timelineScales[name] ?? []).join('·')}
                      </span>
                    )}
                    <button
                      type="button"
          ...
          <div className="mt-3 flex gap-1.5">
            <input
              ...
              placeholder="New timeline (e.g. wikipedia, trips)…"
              title="A timeline is a named time axis. Entries index at their own precision — a bare year at year granularity, a full date at day granularity — nothing to configure."
