...
import { SideViewContext, type SideViewContextValue, type SideViewEntry } from './use-side-view'
...
export function SideViewProvider({ children }: { children: ReactNode }) {
  ...
  const value: SideViewContextValue = {
    entry,
    open: (document: Document, workspaceId: string) => setEntry({ document, workspaceId }),
    close: () => setEntry(null),
  }
