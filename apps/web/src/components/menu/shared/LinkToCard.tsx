...
import { X, Search, Link2, ChevronRight, ChevronDown, FolderPlus, Pin } from 'lucide-react'
...
export function LinkToCard({ onClose, onConfirm, documentCount, fixedWorkspaceName, multiple = true, saving = false, savingContent, sizeClassName, tabs = ['context', 'directory'], title, confirmLabel, onConfirmRelation, relationWorkspaceName, relationPredicates, relationExcludeIds }: LinkToCardProps) {
  ...
  const [activeTab, setActiveTab] = useState<PickerTab | 'pins'>(tabs.includes('context') || tabs.includes('directory') ? 'pins' : (tabs[0] ?? 'context'))
  ...
  const [pins, setPins] = useState<WorkspacePin[] | null>(null)
  const [pinTree, setPinTree] = useState('context')
  const [pinsError, setPinsError] = useState(false)
  const destinationTab = activeTab === 'pins' ? pinTree : activeTab
  useEffect(() => {
    if (!workspaceName) return
    let cancelled = false
    listWorkspacePins(workspaceName).then(rows => { if (!cancelled) setPins(rows) }).catch(() => { if (!cancelled) setPinsError(true) })
  ...
  const pickWorkspace = (name: string) => {
    ...
    setActiveTab(tabs.includes('context') || tabs.includes('directory') ? 'pins' : (tabs[0] ?? 'context'))
  ...
  const confirm = async () => {
    ...
    await onConfirm(Array.from(selected), {
      workspaceName,
      treeName: destinationTab === 'context' ? DEFAULT_WORKSPACE_TREE_NAME : destinationTab,
      treeType: destinationTab === 'context' ? 'context' : 'directory',
    })
  }
      ...
      <div className="flex-1 overflow-hidden">
        ...
        >
          ...
          <div className="flex min-w-0 flex-col overflow-hidden">
            ...
            {activeTab === 'pins' && <div className="min-h-0 flex-1 overflow-y-auto p-3 space-y-2">
              {!pins && !pinsError && <p className="text-xs text-muted-foreground">Loading pins…</p>}
              {pinsError && <p className="text-xs text-destructive">Could not load pins. You can still browse the trees.</p>}
              {!pinsError && pins?.length === 0 && <p className="text-xs text-muted-foreground">No pinned paths yet. Choose a path from a tree.</p>}
              {pins?.filter(pin => pin.resolvable !== false && tabs.includes(pin.tree === DEFAULT_WORKSPACE_TREE_NAME ? 'context' : pin.tree as PickerTab) && `${pin.label ?? pin.name ?? ''} ${pin.path}`.toLowerCase().includes(q)).map(pin => {
                const tab = pin.tree === DEFAULT_WORKSPACE_TREE_NAME ? 'context' : pin.tree
                return <button key={pin.id} type="button" aria-pressed={pinTree === tab && selected.has(pin.path)} className={cn('w-full rounded-md border p-3 text-left text-sm hover:bg-muted', pinTree === tab && selected.has(pin.path) && 'border-primary bg-primary/10')} onClick={() => {
                  if (pinTree !== tab) { setPinTree(tab); setSelected(new Set([pin.path])) }
                  else toggle(pin.path)
                }}><span className="flex items-center gap-2"><Pin className="h-3.5 w-3.5" />{pin.label || pin.name || pin.path}</span><span className="block truncate text-xs text-muted-foreground">{tab} · {pin.path}</span></button>
              })}
            </div>}

            <div className={cn('min-h-0 flex-1 space-y-1.5 overflow-y-auto p-3', (isRelations || activeTab === 'pins') && 'hidden')}>
              {loadingTree ? (
