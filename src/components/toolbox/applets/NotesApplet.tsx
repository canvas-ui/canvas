...
import { useAddTarget, submitDocuments } from '../add/useAddTarget'
import { getWorkspaceDocuments, updateWorkspaceDocument, DEFAULT_WORKSPACE_TREE_NAME } from '@/services/workspace'
...
// The Notes applet: every note visible in the focused context, stacked in one
...
function NoteItem({
  ...
  const scheduleSave = useCallback((nextTitle: string, nextContent: string) => {
    ...
    timer.current = setTimeout(() => save(nextTitle, nextContent), AUTOSAVE_MS)
  }, [save])
...
// Inline creation - a blank note pinned above the list, saved into the
// current target (workspace path or context) like the toolbox NoteForm.
function DraftNote({ onCreated, onCancel }: { onCreated: () => void; onCancel: () => void }) {
...
export function NotesApplet() {
  ...
  const { activeWorkspaceName, activeTreeName, activeContextPath, activeContextType, activeContextId } = state

  const [notes, setNotes] = useState<Document[]>([])
  const [loading, setLoading] = useState(false)
  const [loadError, setLoadError] = useState<string | null>(null)
  // Workspace the update path writes through: the active workspace, or the
  // context's bound workspace (documents always live in a workspace).
  const [updateWorkspace, setUpdateWorkspace] = useState<string | null>(null)
  ...
  const inContext = activeContextType === 'context' && !!activeContextId
  const inWorkspace = !!activeWorkspaceName && !!activeContextPath
  const hasTarget = inContext || inWorkspace
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null)
    try {
      if (inContext && activeContextId) {
        const docs = await getContextDocuments(activeContextId, [NOTE_SCHEMA])
        setNotes(Array.isArray(docs) ? (docs as unknown as Document[]) : [])
        const ctx = await getContext(activeContextId)
        setUpdateWorkspace(ctx.workspaceName || ctx.workspaceId || null)
      } else if (activeWorkspaceName && activeContextPath) {
        const res = await getWorkspaceDocuments(activeWorkspaceName, activeContextPath, [NOTE_SCHEMA], {
          treeName: activeTreeName || DEFAULT_WORKSPACE_TREE_NAME,
          limit: 500,
        })
        setNotes(res.payload || [])
        setUpdateWorkspace(activeWorkspaceName)
      }
    } catch (err) {
  ...
  }, [hasTarget, inContext, activeContextId, activeWorkspaceName, activeContextPath, activeTreeName])

  useEffect(() => { load() }, [load])

  // External creates/edits (AddPanel, other tabs) land here too.
  useEffect(() => {
  ...
  // lands exactly where the hit is (the "click confirms focus/offset" step).
  ...
  if (!hasTarget) {
    return (
      <div className="flex h-full items-center justify-center p-6 text-center text-sm text-muted-foreground">
        Open a workspace path or a context first - Notes shows what lives there.
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {loadError && <p className="px-4 py-6 text-center text-sm text-destructive">{loadError}</p>}
        {!loading && !loadError && !sorted.length && !adding && (
