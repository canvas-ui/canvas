...
export function AgentM2Chat() {
  ...
  const { allMessages, isStreaming, sendMessage, clearMessages, stopStreaming } = useAgentChat({
    agentId,
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        ...
        {allMessages.map((msg, i) => (
          <div
            key={i}
            className={cn(
              'text-xs rounded-lg px-3 py-2 max-w-[90%] whitespace-pre-wrap',
              msg.role === 'user'
          ...
          >
            {msg.content || (msg.role === 'assistant' && !msg.isComplete ? '…' : '')}
          </div>
        ))}
        <div ref={bottomRef} />
      </div>
      ...
      <div className="p-2 border-t border-sidebar-border shrink-0">
        <div className="flex gap-1.5">
          <textarea
            ...
            style={{ height: Math.min(80, Math.max(30, input.split('\n').length * 20)) + 'px' }}
          />
          {isStreaming ? (
            <button
              type="button"
