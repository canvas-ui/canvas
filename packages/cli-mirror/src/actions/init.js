...
export default {
    ...
    async run({ flags, client, session, io }) {
        ...
        if (noStart(flags)) { outro('Configured (not started). `canvas remote mirror start all` when ready.'); return; }
        ...
        if (failed.length) outro(`${failed.length} of ${results.length} mirror(s) did not start — fix the cause above, then \`canvas remote mirror start all\`.`);
        else outro('Check progress with `canvas remote mirror status`; conflicts show up in Workspace › Settings › Sync.');
    },
};
