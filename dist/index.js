...
var DIRECTIONS = ["bi", "pull", "push"];
var SUPERVISORS = ["manual", "pm2", "edge"];
...
function buildMirror({ remoteId, workspaceId, workspaceName, folderName = null, root, folder = null, pins = [], ignore = [], conflicts = "prompt", deletes = "propagate", direction = "bi", stateDir = null, managed = "manual", client = "fuse" }) {
  ...
  return {
    ...
    stateDir: stateDir ? path2.resolve(String(stateDir)) : null,
    client,
...
function configurePublishedMirror({ remoteId, ws: ws2, folder, root, conflicts, deletes, direction = "bi", stateDir = null, managed, ignore = [] }) {
...
async function startMirrors(mirrors, io, { restart = false } = {}) {
  const viaEdge = mirrors.filter((m2) => m2.client === "daemon" || m2.managed === "edge");
  const fuse = mirrors.filter((m2) => m2.client !== "daemon" && m2.managed !== "edge");
  const results = [];
  if (viaEdge.length) {
    for (const m2 of viaEdge) if (m2.paused) upsertMirror({ ...m2, paused: false });
    const managed = viaEdge.some((m2) => m2.client === "daemon" && m2.managed === "pm2") ? "pm2" : "manual";
    const fuseUnits = viaEdge.filter((m2) => m2.client !== "daemon").length;
    try {
      ...
      results.push(...viaEdge.map((m2) => ({ mirror: m2, ok: true })));
    } catch (e) {
      io.error(`canvas-edge: ${e.message}`);
      results.push(...viaEdge.map((m2) => ({ mirror: m2, ok: false, error: e.message })));
    }
  }
  if (fuse.length) {
...
var init_default = {
  ...
  flags: {
    ...
    // propagate | keep
    direction: "string",
    // bi | pull (backup target) | push (one-shot import) — daemon client only
    client: "string",
...
var status_default = {
  ...
  async run({ args, client, io }) {
    ...
    const running = await statusAll().catch(() => []);
    const byMount = new Map(running.map((m2) => [m2.mountpoint, m2]));
    const edge = mirrors.some((m2) => m2.client === "daemon" || m2.managed === "edge") ? await edgeStatus().catch(() => null) : null;
    const edgeById = new Map((edge?.mirrors || []).map((m2) => [m2.id, m2]));
    const rows = [];
    for (const m2 of mirrors) {
      const viaEdge = m2.client === "daemon" || m2.managed === "edge";
      const unit = viaEdge ? edgeById.get(m2.id) : null;
      const st = viaEdge ? unit ? { status: unit.unit === "fuse" ? unit.mount : "ok" } : null : byMount.get(m2.mountpoint);
      ...
      rows.push({
        workspace: m2.workspaceName,
        client: `${m2.client || "fuse"}${m2.managed === "edge" ? "/edge" : ""}`,
...
var stop_default = {
  name: "stop",
  description: "Unmount a mirror (or `all`); the cache and queue stay for the next start (edge-run entries are paused)",
  positional: [{ name: "workspace", required: true }],
  async run({ args, io }) {
    const targets = args.workspace === "all" ? listMirrors() : [findMirror(args.workspace)].filter(Boolean);
    if (targets.length === 0) throw new UsageError(`No mirror for '${args.workspace}'`);
    const daemonTargets = targets.filter((m2) => m2.client === "daemon" || m2.managed === "edge");
    ...
    for (const mirror of targets.filter((m2) => m2.client !== "daemon" && m2.managed !== "edge")) {
      if (mirror.managed === "pm2") {
...
// packages/cli-mirror/src/actions/direction.js
var direction_default = {
  ...
  async run({ args, io }) {
    if (!args.workspace) {
      const rows = listMirrors().map((m2) => ({ mirror: m2.id, client: m2.client, direction: m2.direction || "bi", folder: m2.mountpoint }));
    ...
    if (!mirror) throw new UsageError(`No mirror for '${args.workspace}'`);
    if (!args.direction) {
      io.info(`${mirror.id}: ${mirror.direction || "bi"}`);
      return;
    }
...
// packages/cli-mirror/src/actions/docker.js
var yamlStr = (v) => JSON.stringify(String(v));
var docker_default = {
  name: "docker",
  ...
  async run({ args, flags, client, session: session2, io }) {
    const remoteId = await resolveHub(flags, client, session2, { interactive: false });
    const remote = client.getRemote(remoteId);
    const ws2 = findHubWorkspace(await listHubWorkspaces(client, remoteId), args.workspace);
    if (!ws2) throw new UsageError(`Workspace '${args.workspace}' not found on ${remoteId}`);
    ...
    const folder = flags.folder || `/volume1/work/${ws2.folderName || ws2.name}`;
    const deviceId = flags["device-id"] || `edge-${ws2.name}`.replace(/[^a-zA-Z0-9._-]+/g, "-");
    ...
    const lines = [
      `# ${ws2.folderName || ws2.name} \u2190 ${remoteId} (${direction}); bind the real folder at /data`,
      "services:",
      `  ${name}:`,
      `    image: ${image}`,
      `    container_name: ${name}`,
...
// packages/cli-mirror/src/actions/supervisor.js
var supervisor_default = {
  ...
  async run({ args, flags, io }) {
    ...
    if (targets.length === 0) throw new UsageError(`No mirror for '${args.workspace}'`);
    if (!args.supervisor) {
      io.table(targets.map((m2) => ({ mirror: m2.id, client: m2.client, supervisor: m2.client === "daemon" ? "edge" : m2.managed, folder: m2.mountpoint })));
      return;
    }
    ...
    for (const m2 of targets) {
      if (m2.client === "daemon") {
        io.info(`${m2.id}: a daemon folder is always run by canvas-edge \u2014 skipped`);
        continue;
      }
      if (m2.managed === next) {
        io.info(`${m2.id}: already ${next}`);
        continue;
      }
      if (m2.managed === "pm2") await stopProcess(m2).catch(() => null);
      if (m2.managed === "edge") {
        upsertMirror({ ...m2, paused: true });
        await edgeReload().catch(() => null);
      }
      await unmount(m2.mountpoint).catch(() => null);
      changed.push(upsertMirror({ ...m2, managed: next, paused: false }));
      io.success(`${m2.id}: supervisor ${next}`);
    }
...
var service_default = {
  ...
  async run({ args, io }) {
    ...
    for (const mirror of mirrors.filter((m2) => m2.client !== "daemon" && m2.managed !== "edge")) {
      if (op === "install") {
...
var index_default = {
  ...
  actions: [init_default, add_default, publish_default, remove_default, list_default, status_default, start_default, stop_default, restart_default, sync_default, pin_default, conflicts_default, direction_default, docker_default, supervisor_default, service_default, logs_default, edge_default],
  submodules: [],
