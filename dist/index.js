...
// packages/cli-desktop/src/actions/install.js
...
function treePathSegment(path4) {
  const p = String(path4 || "/");
...
var workspaces = {
  ...
  treePath: (id, treeName, path4) => `/workspaces/${id}/trees/${encodeURIComponent(treeName)}/path${treePathSegment(path4)}`,
...
// packages/cli-desktop/src/lib/release.js
...
// packages/cli-desktop/src/lib/release.js
var REPO = process.env.CANVAS_DESKTOP_REPO || "canvas-ui/canvas";
var APP_DIR = path2.join(CANVAS_HOME, "apps", "desktop");
var STATE = path2.join(APP_DIR, "installed.json");
var PATTERNS = {
...
async function latestRelease() {
  ...
  const releases = (await res.json()).filter((r2) => /^desktop-v/.test(r2.tag_name) && !r2.draft && !r2.prerelease);
  if (releases.length === 0) throw new CanvasError(`No desktop release found on ${REPO}`);
  return releases.sort((a2, b) => new Date(b.created_at) - new Date(a2.created_at))[0];
}
function pickAsset(release, platform = process.platform, arch = process.arch) {
  const pats = PATTERNS[platform] || [];
  const archHint = arch === "arm64" ? /(arm64|aarch64)/i : /(x64|amd64|x86_64)/i;
  const assets = release.assets || [];
  for (const pat of pats) {
    const hits = assets.filter((a2) => pat.test(a2.name));
    const byArch = hits.find((a2) => archHint.test(a2.name)) || hits.find((a2) => !/(arm64|aarch64|x64|amd64|x86_64)/i.test(a2.name)) || hits[0];
...
async function download(asset, dest) {
  ...
  if (/\.(AppImage|app|exe)$/i.test(dest) || process.platform !== "win32") {
    try {
      chmodSync(dest, 493);
    } catch {
    }
...
var install_default = {
  ...
  async run({ flags, io }) {
    ...
    if (!asset) {
      s.stop("No matching asset");
      throw new CanvasError(`${rel.tag_name} has no installer for ${process.platform}/${process.arch}: ${(rel.assets || []).map((a2) => a2.name).join(", ")}`);
    }
    const cur = installed();
    if (!flags.force && cur?.tag === rel.tag_name && cur?.asset === asset.name) {
      s.stop(`${rel.tag_name} already installed (${cur.path})`);
      return;
    }
    s.message(`Downloading ${asset.name} (${Math.round(asset.size / 1048576)} MB)\u2026`);
    const dest = path3.join(APP_DIR, asset.name);
    await download(asset, dest);
    remember({ tag: rel.tag_name, asset: asset.name, path: dest, installedAt: (/* @__PURE__ */ new Date()).toISOString() });
...
// packages/cli-desktop/src/actions/open.js
...
// packages/cli-desktop/src/actions/status.js
var status_default = {
  name: "status",
...
// packages/cli-desktop/src/index.js
var index_default = {
  name: "desktop",
  description: "Canvas desktop app: install (download the release) and open",
  defaultAction: "status",
  needsConnection: false,
  actions: [install_default, open_default, status_default],
  submodules: []
};
export {
